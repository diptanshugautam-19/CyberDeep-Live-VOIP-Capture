...
class DNSdumpster(enumratorBaseThreaded):
    ...
    def get_jwt_token(self, resp):
        jwt_match = re.search(r'"Authorization":\s*"(.*?)"', resp)
        if jwt_match:
    ...
    def enumerate(self):
        ...
        try:
            params = {'target': self.domain}
            post_resp = self.req('POST', 'https://api.dnsdumpster.com/htmld/', params)
        finally:
            if orig_auth:
    ...
    def extract_domains(self, resp):
        try:
            links = re.findall(r'<td>([a-zA-Z0-9\-\.]+\.' + re.escape(self.domain) + ')</td>', resp)
